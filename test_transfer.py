...
def get_asset_info(base_url, api_version, jwt, book_code, asset_number):
    ...
    asset_num = str(pl.get("X_ASSET_NUMBER") or "").strip()
    book = str(pl.get("X_BOOK_TYPE_CODE") or pl.get("P_BOOK_TYPE_CODE") or "").strip()
    ...
    if not asset_id or not asset_num:
        print("\nERROR: Missing identity fields (X_ASSET_ID / X_ASSET_NUMBER) in response.")
        print(f"\nFull ParameterList:\n{json.dumps(pl, indent=2)[:3000]}")
        sys.exit(1)
...
def build_transfer_payload(state, args):
    ...
    if is_noop:
        ...
        print("           Use --location-ccid, --expense-ccid, or --assigned-to to specify new values.")
...
Examples:
  ...
  python test_transfer.py --asset-number 10026 --book-code "OPS CORP" \\
    ...
    print(f"  Book Code    : {args.book_code}")
    ...
    if not args.execute:
        ...
        if is_noop:
            ...
            print("        Specify --location-ccid, --expense-ccid, or --assigned-to to change destination.")
        print("=" * 70)
