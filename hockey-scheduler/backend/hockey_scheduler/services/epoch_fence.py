...
``guardian_service.py`` and ``subtree_deletion_service.py`` names a key the
exact same way, with one source of truth for the string format.
...
it: because ``EPOCH_FENCE_GLOBAL_KEY`` is EXCLUSIVE-locked by all FIFTEEN
...
Player/Guardian reassignment, explicit subtree deletion) for their WHOLE
transaction, any two of those fifteen now serialize against EACH OTHER, not
merely against a concurrent
...
choice to fold all fifteen into one global key (design §4.1/§4.2, §11.4's
...
transaction time of ONE of these fifteen writers (milliseconds), not the
