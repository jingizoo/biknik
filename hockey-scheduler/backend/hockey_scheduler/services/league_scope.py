...
def league_id_for_division(store, division_id: str) -> str:
    ...
    if not season.program_id or store.get_program(season.program_id) is None:
        raise ValidationError(
            "Season is not linked to a valid league.",
            details={
                "reason": "season_league_missing",
                "season_id": season.id,
                "league_id": season.program_id,
            },
        )
...
def league_id_for_game(store, game) -> Optional[str]:
    ...
    if season_id:
        ...
        resolved.add(season.program_id)
    ...
    for team_id in (getattr(game, "home_team_id", None),
        ...
        if team and team.program_id:
            resolved.add(team.program_id)
...
def team_registration_valid(store, season, team_id, division_id=None,
    ...
    if store.get_program(season.program_id) is None:
        return None
    reg = store.registration_for_team_in_season(season.id, team_id)
    if reg is None or not reg.active:
        return None
    team = store.get_team(team_id)
    if team is None or not team.program_id or team.program_id != season.program_id:
        return None
...
def registered_team_ids_in_division(store, division_id):
    ...
    for reg in store.registrations_for_season(division.season_id):
        ...
        if team is None or not team.program_id or team.program_id != league_id:
...
def require_slot_belongs_to_league(store, ice_slot_id: str, league_id: str):
    ...
    """
    league = store.get_program(league_id) if league_id else None
    if league is None:
        raise ValidationError(
    ...
    if (league.operator_organization_id or None) != (venue.organization_id or None):
        raise ValidationError(
            ...
            details={
                ...
                "venue_id": venue.id,
                "league_id": league_id,
                "league_organization_id": league.operator_organization_id,
