...
must stay byte-identical to the pre-C1b contract.
...
"level" shape needs no mapping. Registration and game results never expose the
new competition ``league_id`` (the facade strips it), so they need no mapping
either. Error envelopes (``{"error": {...}}``) pass through unchanged.
"""
