...
class RouteSpec:
    ...
    reassignment_parent_kind / reassignment_parent_field
             The write-side parent-ownership check added when an ``assign-*``
             route attaches an existing record to a parent that the create
    ...
    reassignment_parent_kind: str | None = None
...
REGISTRY = (
    ...
    RouteSpec("GET", r"^/api/officials$", "/api/officials", "get_officials",
              ...
              auth="operator_only",
              scope_axis="none",
              note=("#202: _operator_only('/api/setup/official') -> authz.py -> MANAGE_SCHEDULE. The directory feeds the Official assignment control and is installation-wide; assigned officials remain available to separately-authorized readers through the game-scoped sheet payload. get_officials is self.store.all_officials(), so no Program/Season/League filter applies.")),
    ...
    RouteSpec("POST", r"^/api/admin/subtree-deletion/execute$",
              ...
              note=(
                    ...
                    "requires MANAGE_USERS and SubtreeDeletionService also "
                    "requires exact League Admin plus MANAGE_SETUP and "
                    "MANAGE_USERS before consuming a preview challenge."
              )),
    RouteSpec("POST", r"^/api/admin/subtree-deletion/preview$",
              ...
              auth="session+MANAGE_USERS", scope_axis="none",
              note=(
                    "#429: exceptional inventory-derived preview. This route "
                    "does not change ordinary dependency-gated delete policy."
              )),
    ...
    RouteSpec("POST", r"^/api/setup/division/[^/]+/assign-level$",
              ...
              note=("#202 repair root cause 1: the concrete leaf, not "
                    ...
                    "no writable_parent leg -- this RouteSpec carries no "
                    "reassignment-parent metadata) each judged "
    ...
    RouteSpec("POST", r"^/api/setup/league/[^/]+/assign-organization$",
              ...
              auth="session+MANAGE_SETUP", scope_axis="cross",
              reassignment_parent_kind="organization",
              ...
              note=("#202 repair root cause 1: the concrete leaf. "
                    ...
                    "declared by this RouteSpec's reassignment-parent "
                    "metadata) -- validating every "
    ...
    RouteSpec("POST", r"^/api/setup/player/[^/]+/assign-team$",
              ...
              note=("#202 repair root cause 1: the concrete leaf. "
                    ...
                    "writable_parent leg -- this RouteSpec carries no "
                    "reassignment-parent metadata): the SOURCE Player "
                    "(league -- "
    ...
    RouteSpec("POST", r"^/api/setup/rink/[^/]+/assign-venue$",
              ...
              auth="session+MANAGE_ARENA", scope_axis="cross",
              reassignment_parent_kind="venue",
              ...
              note=("#202 repair root cause 1: the concrete leaf. "
                    ...
                    "on that Venue (setup_parent_writable, declared by this "
                    "RouteSpec's reassignment-parent metadata) -- validating "
                    "every axis either end "
                    "touches")),
    ...
    RouteSpec("POST",
              ...
              note=("#202: authorize(role, path) -- the full path matches "
                    ...
                    "body's own 'division_id' destination is the second "
                    "_guarded_mutation target declared by this RouteSpec; "
                    "the registration's own edge already spans all three "
                    "axes, while the destination pin independently checks "
                    "the named Division")),
    ...
    RouteSpec("POST", r"^/api/setup/team/[^/]+/assign-club$",
              ...
              note=("#202 repair root cause 1: the concrete leaf. "
                    ...
                    "(setup_parent_writable, declared by this RouteSpec's "
                    "reassignment-parent metadata) -- "
                    "validating every axis either end touches")),
...
def reassignment_destination_specs(registry=None):
    ...
    The narrower writable-parent check is defence in depth over the same
    destination. When parent metadata is present, its kind and field must
    therefore agree exactly with this generic target.
    """
    specs = REGISTRY if registry is None else registry
    half_filled = tuple(
        spec.name for spec in specs
    ...
    marked = tuple(
        spec for spec in specs
        if spec.reassignment_destination_kind is not None)
    invalid_values = tuple(
        spec.name for spec in marked
    ...
    invalid_routes = tuple(
        spec.name for spec in marked
        if (spec.method != "POST" or spec.kind != "route"
            or "/assign-" not in spec.template))
    if invalid_routes:
        raise RuntimeError(
            "Reassignment destination metadata is only valid on concrete "
            "POST assign routes: " + ", ".join(invalid_routes))

    missing_routes = tuple(
        spec.name for spec in specs
        if (spec.method == "POST" and spec.kind == "route"
            and "/assign-" in spec.template
            and spec.reassignment_destination_kind is None))
    if missing_routes:
        raise RuntimeError(
            "Concrete POST assign routes require reassignment destination "
            "metadata: " + ", ".join(missing_routes))

    parent_mismatches = tuple(
        spec.name for spec in specs
        if ((spec.reassignment_parent_kind is not None
             or spec.reassignment_parent_field is not None)
            and (spec.reassignment_parent_kind,
                 spec.reassignment_parent_field)
            != (spec.reassignment_destination_kind,
                spec.reassignment_destination_field)))
    if parent_mismatches:
        raise RuntimeError(
            "Reassignment parent and destination metadata must agree on: "
            + ", ".join(parent_mismatches))
    return marked


REASSIGNMENT_DESTINATION_SPECS = reassignment_destination_specs()
_REASSIGNMENT_DESTINATION_MATCHERS = tuple(
    (spec, re.compile(spec.pattern))
    for spec in REASSIGNMENT_DESTINATION_SPECS)


def reassignment_parent_specs(registry=None):
    """Return assign-route contracts needing a writable-parent target.

    Parent kind and request-body field are one policy fact. A half-populated,
    mistyped, empty, or structurally misplaced declaration is a registry
    defect and must stop request handling rather than silently omit the
    destination-parent authorization check.
    """
    specs = REGISTRY if registry is None else registry
    half_filled = tuple(
        spec.name for spec in specs
        if ((spec.reassignment_parent_kind is None)
            != (spec.reassignment_parent_field is None)))
    if half_filled:
        raise RuntimeError(
            "Reassignment parent kind/field must be paired on: "
            + ", ".join(half_filled))

    marked = tuple(
        spec for spec in specs if spec.reassignment_parent_kind is not None)
    invalid_values = tuple(
        spec.name for spec in marked
        if (type(spec.reassignment_parent_kind) is not str
            or not spec.reassignment_parent_kind.strip()
            or type(spec.reassignment_parent_field) is not str
            or not spec.reassignment_parent_field.strip()))
    if invalid_values:
        raise RuntimeError(
            "Reassignment parent metadata is invalid on: "
            + ", ".join(invalid_values))
    ...
    if invalid_routes:
        raise RuntimeError(
            "Reassignment parent metadata is only valid on concrete POST "
            "assign routes: " + ", ".join(invalid_routes))
    return marked


REASSIGNMENT_PARENT_SPECS = reassignment_parent_specs()
_REASSIGNMENT_PARENT_MATCHERS = tuple(
    (spec, re.compile(spec.pattern)) for spec in REASSIGNMENT_PARENT_SPECS)
...
def runtime_reassignment_parent_spec(path: str, registry=None):
    """Return the writable-parent contract matching POST ``path``.

    ``None`` means this reassign does not share a write-side parent rule with
    a create route. More than one match is a policy ambiguity and fails closed
    rather than choosing a request-body field by registry order.
    """
    path = path.split("?", 1)[0]
    if registry is None:
        matches = [spec for spec, pattern in _REASSIGNMENT_PARENT_MATCHERS
                   if pattern.match(path)]
    else:
        matches = [spec for spec in reassignment_parent_specs(registry)
                   if re.match(spec.pattern, path)]
    if len(matches) > 1:
        raise RuntimeError(
            "Ambiguous RouteSpec reassignment parent for " + path)
    return matches[0] if matches else None


def runtime_reassignment_destination_spec(path: str, registry=None):
    """Return the generic destination contract matching POST ``path``.

    Every concrete assign route has exactly one such contract. More than one
    match is a policy ambiguity and fails closed rather than selecting a kind
    or request-body field by registry order.
    """
    path = path.split("?", 1)[0]
    if registry is None:
        matches = [
            spec for spec, pattern in _REASSIGNMENT_DESTINATION_MATCHERS
            if pattern.match(path)]
    else:
        matches = [
            spec for spec in reassignment_destination_specs(registry)
            if re.match(spec.pattern, path)]
    if len(matches) > 1:
        raise RuntimeError(
            "Ambiguous RouteSpec reassignment destination for " + path)
    return matches[0] if matches else None
