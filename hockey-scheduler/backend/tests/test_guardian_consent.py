...
class GuardianConsentHttpTest(unittest.TestCase):
    ...
    def _login(self, username, password="demo"):
        ...
        # #409: the Player CREATE these tests use is guarded, so it needs an
        # EXPLICIT context. Persist byte-for-byte the tuple this session
        # already resolves, so the Player lands on exactly the seeded Team it
        # always did.
