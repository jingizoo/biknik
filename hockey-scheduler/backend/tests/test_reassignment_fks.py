    ...
    with store.transaction():
        ...
        if store.backend == "postgres":
            ...
            cur.execute("CREATE INDEX IF NOT EXISTS ix_teams_league "
                        "ON teams (league_id)")
            # The rebuild above recreates `players` at its pre-058 shape (the
            # hardcoded pre-#273 column list), so 058's identity columns are
            # physically gone while still recorded in the ledger. Un-record
            # 058 too: the re-migrate then replays 040 AND 058 in order,
