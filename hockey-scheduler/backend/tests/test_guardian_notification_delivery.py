...
def _seeded_api():
    ...
    api.setup.grant_season_venue_access(season.id, venue.id)
...
class GuardianPreferenceHttpTest(unittest.TestCase):
    ...
    def _login(self, username):
        ...
        # #409: the Player CREATE below is guarded, so persist byte-for-byte
        # the tuple this session already resolves.
