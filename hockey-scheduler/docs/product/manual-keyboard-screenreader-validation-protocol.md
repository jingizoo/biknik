...
- The already-implemented mechanics this protocol tests against, as
  ...
  the native `<select id="ctx-select">` (inside the `#context-switcher`
  wrapper) context control; the second, independent native
  `<select id="ctx-league-select">` League control beside it; and context
  *filtering* itself — the operational reads behind Games, Roster,
  Standings, the Dashboard and the Setup summaries all resolve the active
  `(Program, Season, League)` server-side and are scoped to it (#367/#369).
  The persistent `#ctx-scope-note` beside the two selects is the operator-
  facing statement of that, and is wired as `aria-describedby` on both.

  **Superseded — do not run the earlier gating.** Until this revision, this
  section and K5/S5 below told a validator that context filtering was *not*
  implemented, that `main` carried a `display only · screens not filtered`
  caption, and that "no visible content change on switch" was the correct
  PASSING result. All three statements described a head that predates
  #367/#369 and are false on any head carrying this file. A pass run against
  the old wording would have recorded the opposite of shipped behaviour and
  failed a correct implementation, which is why K5/S5 are restated below in
  terms of what the application actually does.
...
| --- | --- | --- | --- |
| 2026-07-27 | K1/K2 (skip link) | Initial draft required a focusable skip link on the signed-out login/Public Schedule shells, contradicting the accepted fix that hides `#skip-link` atomically with `.web` on those surfaces | **Resolved 2026-07-27** — corrected to assert no focusable skip link signed-out, and require it only on the first Tab of the authenticated shell |
| 2026-07-27 | K7/K8 (dialog entry) | Initial draft only required focus to stay "somewhere inside" the dialog on the first Tab/Shift+Tab, which could pass a broken entry boundary | **Resolved 2026-07-27** — corrected to require the exact first/last sequential control, checked independently on two separate fresh opens |
| 2026-07-27 | K15 (retry focus) | Initial draft required focus to always remain on the retried control, which is impossible when a successful retry's rerender removes that control | **Resolved 2026-07-27** — corrected to define both outcomes: re-resolved-and-retained if the control survives, named fallback if it doesn't |
| 2026-07-27 | K5/S5 (context selector) | Initial draft named `#context-switcher` as the native select (it is the wrapper; the select is `#ctx-select`) and implied context filtering was already implemented, when `main` still shows the "display only · screens not filtered" caption | **Resolved 2026-07-27** — corrected selector names throughout; filtering-specific assertions now explicitly gated on confirming the caption has been removed/narrowed on the head under test |
| 2026-08-04 | K5/S5 (context selector) | The 2026-07-27 resolution above inverted with the code and became the blocker. #367/#369 shipped the filtering, but K5/S5 still instructed the validator that no content re-filters and that this was the EXPECTED PASSING RESULT — so running the human pass as written would have recorded the opposite of shipped behaviour, and a correct implementation would have been logged as a failure. This is why the human pass was blocked rather than merely pending. | **Resolved 2026-08-04** — K5/S5 restated in terms of observed application behaviour, with no gate on caption text at all. The gate was the defect: it tied a manual accessibility step to a literal sentence in `index.html`, so ordinary copy drift silently changed what a validator was told to expect. The row above is left verbatim as the historical record of the earlier (opposite) correction. |
...
| --- | --- | --- |
| K1 | From a fresh page load (signed out, on the login card), press Tab repeatedly through the full card. Separately, repeat on the Public Schedule surface. | **No focusable skip link appears in either tab order.** `#skip-link` lives inside the authenticated `.web` shell, which `body.signed-out .web { display: none }` hides atomically on both surfaces — a focusable skip link here (with its `#content` target hidden) is the dangling-link defect #345 already fixed, not a passing result. |
| K2 | Sign in, reach the authenticated shell (reload it if needed so this is a fresh boot of `.web`), and press Tab once. | Focus lands on the **"Skip to main content"** link (`#skip-link`), which becomes visually visible. This must be the first relevant tab stop in the authenticated shell — before any nav/chrome control. Then activate it (Enter): focus moves into `#content` (not just a scroll) — a real focus move, verified by checking `document.activeElement`. |
...
| K7 | Open a dialog from a fresh trigger activation (not chained from K8's run). Immediately after open, confirm focus is on the dialog *container* itself (`document.activeElement` is the `role="dialog"` element, carrying `tabindex="-1"` — per `syncOverlayFocus()`/`focusOverlayContainer()`). Then press Tab once. | Focus moves to **exactly the first sequential focusable control** inside the dialog (the first element `overlayFocusables()` returns) — not merely "somewhere inside," and not left on the container. This is the entry-boundary's forward case. |
| K8 | Open a **separately, freshly opened** dialog (its own independent open, not reused from K7 or chained after it). Immediately after open (focus still on the container), press Shift+Tab once. | Focus moves to **exactly the last sequential focusable control** inside the dialog. This is the entry-boundary case #345 fixed (`df244ab`) after finding the first Shift+Tab used to escape to the page behind the drawer — before the fix, the container was excluded from `overlayFocusables()` so neither the forward-wrap nor backward-wrap check ever matched it, and native backward traversal from a `tabindex="-1"` node walked to whatever preceded it in the document (out of the dialog). Landing anywhere other than the exact last control (including "stayed inside the dialog but not on the last control") is a fail. |
