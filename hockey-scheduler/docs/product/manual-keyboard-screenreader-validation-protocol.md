...
- The already-implemented mechanics this protocol tests against, as
  ...
  wrapper) context control. **Not yet implemented**: context *filtering* —
  `main` still carries the literal `<span class="ctx-unfiltered">display
  only · screens not filtered</span>` caption (`index.html`), and League has
  not yet been promoted into the persistent bar. Those are #345
  requirements still pending on the implementation PR, not mechanics
  already in place — see K5/S5 below for how this protocol treats that
  distinction.
...
| K5 | Change the active Program/Season via `#ctx-select` (inside the `#context-switcher` wrapper) using only the keyboard (arrow keys / typeahead on the native `<select>`). | The selection updates and focus remains on `#ctx-select` — it does not jump elsewhere unexpectedly. **On the current `main` head, the active screen does not re-filter** — the persistent "display only · screens not filtered" caption is accurate today, so "no visible content change" is the correct (passing) result, not a defect. **Precondition for testing filtering itself:** only assert that the active screen re-filters once #345's context-filtering requirement has actually landed on the head under test (confirm the `ctx-unfiltered` caption/copy has been removed or narrowed first) — running this check against `main` as it stands today and expecting a filter change would fail a correct implementation. |
...
| S5 | Change the active Program/Season via `#ctx-select`. | The screen reader announces the newly selected option (native `<select>` semantics — confirm this is preserved, since #345 requires keeping the native control specifically for this reason). **Test the "content re-filters and the change is announced" half of this step only once #345's context-filtering requirement has actually landed** (confirm the current `ctx-unfiltered`/"display only" caption has been removed or narrowed on the head under test first) — on `main` as it stands today, no content re-filters, so there is nothing to announce, and that is the correct (passing) result rather than a gap to flag. |
